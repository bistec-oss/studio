...
function SearchPalette({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <div className="fixed top-[15%] left-1/2 -translate-x-1/2 z-[201] w-[calc(100vw-2rem)] max-w-[520px] animate-scale-in">
...
export default function Header({ title, breadcrumbs }: { title?: string; breadcrumbs?: Breadcrumb[] }) {
  ...
  return (
    <>
      <header className="px-4 md:px-6 h-14 border-b border-white/[0.06] flex items-center justify-between gap-3 flex-shrink-0 bg-surface-1/80 backdrop-blur-md">
        <div className="flex items-center gap-2">
          {breadcrumbs ? (
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.03] border border-white/[0.05] text-slate-500 text-[0.78rem] cursor-pointer md:min-w-[200px] hover:border-white/[0.1] transition-colors text-left"
          >
            <Search size={13} className="opacity-50" />
            <span className="hidden sm:inline">Search...</span>
            <kbd className="ml-auto hidden md:inline font-mono text-[0.58rem] px-1.5 py-0.5 rounded-md bg-white/[0.04] text-slate-600 border border-white/[0.06]">{'\u2318'}K</kbd>
          </button>
