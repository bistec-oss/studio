...
2. **The image never names the programme.** The headline is "REAL INDUSTRIAL TRAINING" and the navy band reuses "A real industrial training programme" — which the briefing itself calls a _description, not the name_. Only the caption carries the real name.

### Both fixed — briefing v2 + kit prompt v3 (2026-07-28)

**The naming miss was the briefing's fault, not the model's.** Two clauses combined badly: the naming rule required the full name only "on first mention **in the caption**" and merely said "IRP is fine on-image" — _permitted_, never _required_ — while the same briefing blessed "a real industrial training programme" as acceptable supporting copy, lifted from the reference creative's navy band. So the most name-like phrase available to the design agent was the descriptor, and it promoted that to the headline. It obeyed the caption rule exactly and had nothing forcing the name into the image. **Lesson: anything that must appear in the rendered image has to be stated as a requirement on the image, not as a permission.**

- **Campaign briefing v2** — NAMING rewritten to "the programme must be NAMED, not described": the headline (the largest text on the canvas) **must** contain "INDUSTRY READINESS PROGRAMME" or "IRP", framed as "a reader who sees only the image must come away knowing what the programme is called"; substitute names such as "Real Industrial Training" are explicitly forbidden as headlines; descriptors are demoted to a smaller supporting subline only. The LAYOUT headline bullet carries the same requirement inline, with an example.
- **Kit voice prompt v3** — the logo lockup is now specified **geometrically rather than adjectivally**, because "similar visual heights / generous clear space" produced a full-width bar with the two logos shoved into opposite corners and the Hearts mark shrunken: a **compact, horizontally centred** pill that **hugs** the logos (~45–60% of canvas width, never full-bleed), the two logos **close together in the middle** (gap ≈ one or two letters of the wordmarks, small equal padding either side, reading as one unit), both at the **same optical height** (~6–8% of canvas height, judged on the visible mark rather than the bounding box, with the Hearts mark named explicitly so it cannot shrink again).
...
> **⚠️ Deployment caveats:** (a) run `npx prisma migrate deploy` (two new migrations). (b) ~~Scheduled generation in the Docker scheduler container requires API mode — the image has no `claude` CLI~~ **Superseded 2026-07-07 (per-user-token feature):** the image now ships the Claude Code CLI, so container scheduled generation works under `DESIGN_PROVIDER=cli` too (always on the shared `CLAUDE_CODE_OAUTH_TOKEN`; the worker's startup warning now fires only when that token is missing). (c) Scheduler-created briefs use `copyProviderKey: 'env-default'` (falls through to the default enabled COPY provider → env key), same as MCP.
