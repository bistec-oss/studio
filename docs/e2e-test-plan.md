...
- **TC-AGUI-05b — Restore refuses a non-positive revision number.** S: POST `…/revisions/{-1,-2,0}/restore`. E: 400 each; draft untouched. The rejected-render number space is negative (below), so accepting any integer would have let a caller restore a render that was deliberately not applied.
- **TC-AGUI-06 — Refine ownership.** P: editor B, draft owned by A. E: 403 (rewritten 2026-07-21 to D6 — a campaign-shared draft is a 202 for an in-team editor; a private one is a 404). **Guards H2.**

#### E2. Design-instruction fidelity (change 004)

> ⚠️ **Read the boundary before extending these.** The suite runs in **API mode** (`.env.test` → `DESIGN_PROVIDER=claude-html`), where the refine route carries **no classification envelope** and always takes FR-05's preserving default (`classes:['add']`, `supersedes:[]`). And `MOCK_AI` is a **stub**: `buildMockHtml` makes a mocked refine observably an _edit_ (a digest of the prompt is folded into the document) but it does **not obey the instruction**. So nothing in this section can measure _what the model did_ — only _what the pipeline does about it_. The failure branches are reachable only through T12's deterministic seam (`shouldMockVerificationMiss`): `__FAIL_VERIFY_ALWAYS__` in the instruction misses both attempts, `__FAIL_VERIFY_ONCE__` misses once and passes the retry (must be **unique per draft**, like `__FAIL_ONCE__`).
>
> **Only a real CLI-mode run can prove:** AC-08's "measurably shorter", AC-09's "the superseded element is absent" (and the duplicate-image export failing it), AC-10 (destructive class with empty `supersedes` downgrades), AC-11 (multi-class), AC-12's zero-model-call post-conditions, AC-13's single `add` call. Those live in the unit tests over `resolveEffectiveClasses` / `instructionClasses` / `refineVerify`, plus a live smoke test. **Do not label a case here with one of those AC numbers.**

- **TC-AGUI-07 — Twice-missed refine is NOT applied (reduce-the-text regression).** P: EXPORTED draft. S: refine `Reduce the text __FAIL_VERIFY_ALWAYS__ […]` → 202 → `waitForAction`. E: `notAppliedReason` set (a string); `pendingActionError` **null** (the run did not crash); `htmlContent` + `currentRevisionNumber` byte-identical to before; the instruction appears in **no** `/revisions` row. **Regression for reported defect #1.** _Partial AC-08 — the outcome half only._
- **TC-AGUI-08 — A rejected render never reaches the user (use-the-uploaded-image regression).** S: refine `Use the uploaded image as the background __FAIL_VERIFY_ALWAYS__ […]`. E: the draft still points at the **previous** export object (compare URL _path_ — the signature differs per read); `htmlContent` unchanged; `notAppliedReason` set. **Regression for reported defect #2.** _Partial AC-09 — the outcome half only._
- **TC-AGUI-09 — Miss once, pass on the retry → commits normally.** S: refine `… __FAIL_VERIFY_ONCE__ [draftId]`. E: new chain revision carrying that instruction; `currentRevisionNumber` 1→2; both failure fields null. Proves the retry is a genuine **second attempt** (without it this instruction would land on the not-applied path).
- **TC-AGUI-10 — A twice-failed refine leaves the chain and pointer untouched (AC-16).** P: one successful refine first (chain at v2). S: a `__FAIL_VERIFY_ALWAYS__` refine. E: `/revisions` same length, numbers still **positive and contiguous from 1**; `revisionCount` and `currentRevisionNumber` unmoved; `htmlContent` unchanged. Also asserts the retained rejected render is **unreachable**: `…/revisions/-1/restore` and `/-2/restore` → 400. **AC-17 note:** the rejected row has _no read surface over HTTP_ by design (chain-filtered out of `/revisions` and `revisionCount`, number space rejected by restore), so its stored instruction/classes/`rejectionReason` are **not assertable from this suite** — only via a DB read.
- **TC-AGUI-11 — The poll's three outcomes are distinguishable (AC-18).** S: on ONE draft, in sequence — (1) a normal refine, (2) `__FAIL_GEN_ALWAYS__` (the run throws), (3) `__FAIL_VERIFY_ALWAYS__`, (4) a normal refine. E: (1) both fields null + new revision; (2) `pendingActionError` set, `notAppliedReason` null; (3) `notAppliedReason` set, `pendingActionError` null, no revision; (4) both back to null — `claimDraftAction` retires the stale not-applied card.
