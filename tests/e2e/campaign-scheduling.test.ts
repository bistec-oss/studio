...
test.describe('Campaign page UI — briefing + queue sections', () => {
  ...
  async function pageLogin(page: Page) {
    ...
    // on /choose-team instead of the dashboard. A fresh full navigation
    // (not the client router's soft nav, which can settle on a stale/optimistic
    // "/" before the server-evaluated redirect lands) forces a real SSR
    // round-trip so team resolution is evaluated against the cookie set
    // moments ago by sign-in.
    await page.goto('/')
