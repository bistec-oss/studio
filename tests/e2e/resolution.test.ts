...
test.describe('Brand-kit resolution', () => {
  ...
  async function makeKit(client: ApiClient, name: string) {
    return (await client.post('/api/admin/brandkits', { name, colors: ['#0284c7'] })).json()
  }

  // TC-RES-01 — Campaign override wins over the project default.
  test('campaign kit takes precedence over the project default', async () => {
    const projKit = await makeKit(api, 'Project Kit X')
    const campKit = await makeKit(api, 'Campaign Kit Y')
    const project = await (await api.post('/api/projects', { name: 'Res P1', defaultBrandKitId: projKit.id })).json()
    const campaign = await (await api.post('/api/campaigns', {
      name: 'Res C1', brandKitId: campKit.id, projectId: project.id,
    })).json()

    const { kit, source } = await (await api.get(`/api/campaigns/${campaign.id}/brandkit`)).json()
    expect(source).toBe('campaign')
    expect(kit.id).toBe(campKit.id)
  })

  // TC-RES-02 — Inherit from the project when the campaign has no kit.
  test('campaign with no kit inherits the project default', async () => {
    const projKit = await makeKit(api, 'Project Kit X2')
    const project = await (await api.post('/api/projects', { name: 'Res P2', defaultBrandKitId: projKit.id })).json()
    const campaign = await (await api.post('/api/campaigns', { name: 'Res C2', projectId: project.id })).json()
  ...
  test('a standalone campaign falls back to the system default kit', async () => {
    const campaign = await (await api.post('/api/campaigns', { name: 'Res C3 standalone' })).json()
    const { kit, source } = await (await api.get(`/api/campaigns/${campaign.id}/brandkit`)).json()
  ...
  test('a soft-deleted campaign kit is skipped and resolution falls through', async () => {
    const projKit = await makeKit(api, 'Project Kit X4')
    const deletedKit = await makeKit(api, 'Soon-Deleted Kit Y4')
    const project = await (await api.post('/api/projects', { name: 'Res P4', defaultBrandKitId: projKit.id })).json()
    const campaign = await (await api.post('/api/campaigns', {
      name: 'Res C4', brandKitId: deletedKit.id, projectId: project.id,
    })).json()

    // Soft-delete the campaign's kit.
    expect((await api.del(`/api/admin/brandkits/${deletedKit.id}`)).status()).toBe(204)
  ...
  test('an editor cannot reassign a campaign to a project', async ({ request }) => {
    const project = await (await api.post('/api/projects', { name: 'Res P5' })).json()
    const campaign = await (await api.post('/api/campaigns', { name: 'Res C5' })).json()
  ...
  test('campaign and project lists are bounded arrays', async () => {
