...
// Unfinished-brief autosave. Strictly owner-scoped (withTeamAuth, no admin
...
export const GET = withTeamAuth(async (_req, _ctx, user) => {
  const rows = await listBriefDrafts(user.userId, user.teamId)
...
export const PUT = withTeamAuth(async (req: NextRequest, _ctx, user) => {
  const body = await parseBody(req, putSchema)
  if (body.response) return body.response

  const result = await saveBriefDraft(user.userId, body.data.id, body.data.payload, user.teamId)
  if (!result.ok) {
