...
**Problem:** "Detect dropped" only covers the case where a placeholder is absent. A model that _mangles_ a placeholder — renames it, changes its index, duplicates it, or wraps it — produces a document where detection finds nothing to restore and the revision commits with the image still lost or, worse, with the wrong asset spliced into the wrong slot at render time. The recovery is silently a no-op and looks identical to a clean run, which is the same shape as the class of bug this proposal is written to close.
