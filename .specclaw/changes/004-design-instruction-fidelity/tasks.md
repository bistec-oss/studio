...
- [x] `T1` — Install monochrome symbol font coverage in the runner image
  - Files: `Dockerfile`
  - Estimate: small
  - Kind: config
...
- [x] `T2` — Font-set identifier, stamped per draft
...
- [x] `T3` — Reusable rasterizing test harness with glyph-agnostic tofu assertion
...
- [x] `T4` — Inline-asset reconciliation replacing detection
...
- [x] `T5` — The single per-class table
...
- [x] `T7` — Verification module with three-state result
...
- [x] `T8` — Phase 2 migration: not-applied outcome and rejected-render retention
...
- [x] `T9` — Exclude rejected revisions from every consumer
...
- [x] `T10` — Wire classification, verification, retry-once and the not-applied outcome into the refine route
...
- [x] `T11` — Surface not-applied as a failure in the UI
...
- [x] `T12` — Deterministic verification-miss seam
...
- [x] `T13` — E2E coverage for the refine fidelity contract
...
- [x] `T14` — Closed input grammar for element edits
...
- [x] `T15` — Element-scoped write path on the existing inline-edit route
...
- [x] `T16` — Click-to-select element mode in the editor, with E2E
