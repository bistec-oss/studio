...
- Gates: tsc, lint (0 errors, 7 pre-existing warnings), **174/174 unit**, full mock **E2E 145 passed / 4 skipped / 0 failed**, production build. **⚠️ Deploy:** `npx prisma migrate deploy` (1 new migration `20260717085058`); no new env vars. **Machine-1 TODO:** run `node --env-file=.env scripts/fix-data-uri-logos.mjs --dry-run` then real (its Hearts kit has the same data-URI rows).
