...
- **TC-REG-H7a rewritten** to the single-flight contract (10 sequential refines → contiguous 2..11; parallel fire → exactly one 202, rest 409); §Q suite (9 cases) + TC-BK-09 added; `agui-refinement` rewritten async. Also fixed 3 pre-existing lint errors in `scripts/export-posts.mjs` and restored `BISTEC_API_KEYS` to `.env.test`.
...
- **✅ Live-verified same day (real dev server, CLI Claude, no mocks):** brief-draft autosave → Resume (full rehydration incl. step/campaign/kit) → Discard, all in a real browser; async regenerate-copy 202 → `pendingAction` → new copy → Undo restore; async refine proven by six real revisions the same afternoon; brand-kit doc upload → live chat quoted a marker from the doc → delete; Hearts-kit prompts back to ~5k chars (vs 141k/277k incident), regenerate-design 41s vs the 300s timeout. (Testing note: DevTools-protocol `fill` doesn't fire React `onChange` on textareas — use real keystrokes when browser-testing autosave.)
